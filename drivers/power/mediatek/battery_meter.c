...
extern kal_int32 pre_sync_soc;
static kal_bool sync_soc_status = KAL_FALSE;
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */

/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/16/2015, pr-1091177, calibration boot soc */
static kal_int32 gFG_capacity_original = 0;
static kal_int32 gFG_init_voltage = 0;
static kal_int32 gFG_boot_capacity = 0;
static kal_int32 vbat_boot_voltage = 0;
static kal_int32 vbat_boot_capacity = 0;
static kal_int32 gFG_voltage_calibration = 0;
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/24/2015, pr-1092300, add battery manufacturer check */
    ...
    if ((id_volt > BATTERY_SCUD_ID_MIN_UV) && (id_volt < BATTERY_SCUD_ID_MAX_UV)){
        g_fg_battery_id = 1;
    } else {
        g_fg_battery_id = 0;
    }
...
#if defined(MAX_LIMIT_CHARGE_TEMPERATURE)
    batt_cust_data.max_limit_charge_temperature = MAX_LIMIT_CHARGE_TEMPERATURE;
#endif

#if defined(MAX_RAISING_CHARGE_TEMPERATURE)
    batt_cust_data.max_raising_charge_temperature = MAX_RAISING_CHARGE_TEMPERATURE;
#endif

#if defined(MIN_DROPPING_CHARGE_TEMPERATURE)
    batt_cust_data.min_dropping_charge_temperature = MIN_DROPPING_CHARGE_TEMPERATURE;
#endif
...
#endif
#if defined(V_CHARGER_NOL)
    batt_cust_data.v_charger_nol = V_CHARGER_NOL;
#endif
...
#if defined(CONFIG_OF) && defined(BATTERY_DTS_SUPPORT)
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
	...
	bm_print(BM_LOG_CRTI, "[fgauge_get_Q_max] temperature = %d, low_temperature = %d, low_Q_max = %d, high_temperature = %d, high_Q_max =%d, g_fg_battery_id =%d\n",
			 temperature, low_temperature, low_Q_max, high_temperature, high_Q_max, g_fg_battery_id);
...
{
	printk("<fgauge_get_profile>  temperature = %d, charge_status = %d\n",
			temperature, bat_is_charger_exist());
#ifdef TCT_BATTERY_PROFILE_SEPARATION
	if (bat_is_charger_exist() == KAL_TRUE){
		...
		switch (temperature) {
	    case TEMPERATURE_T0:
	        return &battery_profile_discharge_t0[g_fg_battery_id][0];
	        break;
	    case TEMPERATURE_T1:
	        return &battery_profile_discharge_t1[g_fg_battery_id][0];
	        break;
	    case TEMPERATURE_T2:
	        return &battery_profile_discharge_t2[g_fg_battery_id][0];
	        break;
	    case TEMPERATURE_T3:
	        return &battery_profile_discharge_t3[g_fg_battery_id][0];
	        break;
	    case TEMPERATURE_T:
	        return &battery_profile_temperature[0];
	        break;
...
{
	printk("<fgauge_get_profile_r_table>  temperature = %d, charge_status = %d\n",
			temperature, bat_is_charger_exist());
#ifdef TCT_BATTERY_PROFILE_SEPARATION
	if (bat_is_charger_exist() == KAL_TRUE){
		switch (temperature) {
		...
		switch (temperature) {
		case TEMPERATURE_T0:
		    return &r_profile_discharge_t0[g_fg_battery_id][0];
		    break;
		case TEMPERATURE_T1:
		    return &r_profile_discharge_t1[g_fg_battery_id][0];
		    break;
		case TEMPERATURE_T2:
		    return &r_profile_discharge_t2[g_fg_battery_id][0];
		    break;
		case TEMPERATURE_T3:
		    return &r_profile_discharge_t3[g_fg_battery_id][0];
		    break;
		case TEMPERATURE_T:
		    return &r_profile_temperature[0];
		    break;
...
#if 0 //defined(SW_OAM_INIT_V2)		/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/16/2015, pr-1091177, calibration boot soc */
    sw_oam_init_v2();
#endif

    bm_print(BM_LOG_CRTI, "[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d, poweron_tolrance =%d\n",
             g_rtc_fg_soc, gFG_capacity_by_v, batt_meter_cust_data.cust_poweron_delta_capacity_tolrance);

/*[BUGFIX]-Add-BEGIN by TCTSZ.leo.guo, 07/24/2015, soc sync time synchro */
#ifdef SOC_SYNCHRO_UI_SOC_TIMER
	if (sync_soc_status == KAL_FALSE) {
		pre_sync_soc = gFG_capacity_by_v;
		sync_soc_status = KAL_TRUE;
	}
	printk("<synchro>  pre_sync_soc = %d, g_rtc_fg_soc = %d, gFG_capacity_by_v = %d\n",
			pre_sync_soc, g_rtc_fg_soc, gFG_capacity_by_v);
#endif
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/16/2015, pr-1091177, calibration boot soc */
    kal_int32 gfg_count = 0;
	kal_int32 gfg_vol_sum = 0;
	kal_int32 vbat_count = 0;
	kal_int32 vbat_vol_sum = 0;
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */

    /*stop charging for vbat measurement */
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 10/20/2015, pr-742581, stop charging for vbat measurement */
	if(bat_is_charger_exist() == KAL_TRUE) {
...
#else
	for (gfg_count = 0; gfg_count < 5; gfg_count++) {
		ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);
		gfg_vol_sum = gfg_vol_sum + gFG_voltage;
		msleep(5);
	}
	gFG_voltage = gfg_vol_sum / gfg_count;

	for (vbat_count = 0; vbat_count < 5; vbat_count++) {
		ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_SENSE, &g_booting_vbat);
		vbat_vol_sum = vbat_vol_sum + g_booting_vbat;
		g_booting_vbat = 5;
		msleep(5);
	}
	g_booting_vbat = vbat_vol_sum / vbat_count;

	gFG_init_voltage = gFG_voltage;
	vbat_boot_voltage = g_booting_vbat;
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
	gFG_capacity_original = gFG_capacity_by_v;
...
/********add by zero ********************/
	...
	printk("zero---gFG_voltage=%d,g_booting_vbat = %d\n",gFG_voltage, g_booting_vbat);
	 ...
	 if(bat_is_charger_exist() == KAL_TRUE)
	 {
		if (abs(gFG_voltage - g_booting_vbat) >= 40) {
			if(g_booting_vbat < 3350)
				g_booting_vbat = g_booting_vbat + 100;
			else if(g_booting_vbat >= 3350 && g_booting_vbat < 3450)
				g_booting_vbat = g_booting_vbat + 80;
			else if(g_booting_vbat >= 3450 && g_booting_vbat < 3550)
				g_booting_vbat = g_booting_vbat + 70;
			else if(g_booting_vbat >= 3550 && g_booting_vbat < 3650)
				g_booting_vbat = g_booting_vbat + 60;
			else if(g_booting_vbat >= 3650 && g_booting_vbat < 3750)
				g_booting_vbat = g_booting_vbat + 55;
			else if(g_booting_vbat >= 3750 && g_booting_vbat < 3850)
				g_booting_vbat = g_booting_vbat + 55;
			else if(g_booting_vbat >= 3850 && g_booting_vbat < 3950)
				g_booting_vbat = g_booting_vbat + 50;
			else if(g_booting_vbat >= 3950 && g_booting_vbat < 4050)
				g_booting_vbat = g_booting_vbat + 50;
			else if(g_booting_vbat >= 4050 && g_booting_vbat < 4150)
				g_booting_vbat = g_booting_vbat + 45;
			else if(g_booting_vbat >= 4150)
				g_booting_vbat = g_booting_vbat + 40;

			gFG_voltage = g_booting_vbat;
		}
		else {
			if(gFG_voltage > 3500 && gFG_voltage < 3700)
				gFG_voltage = gFG_voltage - 10;
				else if(gFG_voltage >= 3700 && gFG_voltage < 3900)
					gFG_voltage = gFG_voltage - 5;
			else if(gFG_voltage >= 3900 && gFG_voltage < 4350)
				gFG_voltage = gFG_voltage - 10;
		}
	 }
	 else
	...
	gFG_voltage_calibration = gFG_voltage;
	gFG_boot_capacity = gFG_capacity_by_v;
	vbat_boot_capacity = vbat_capacity;
...
void oam_run(void)
{
	...
	bm_print(BM_LOG_CRTI, "[oam_init] gFG_init_voltage = %d, gFG_capacity_original = %d, vbat_boot_voltage = %d, vbat_boot_capacity = %d\n",
		 gFG_init_voltage, gFG_capacity_original, vbat_boot_voltage, vbat_boot_capacity);

	bm_print(BM_LOG_CRTI, "[oam_init] gFG_voltage_calibration = %d, gFG_boot_capacity = %d, g_rtc_fg_soc = %d\n",
		 gFG_voltage_calibration, gFG_boot_capacity, g_rtc_fg_soc);
}
...
kal_int32 battery_meter_get_charging_current(void)
{
	...
	return 0;
/* [PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, 2015.07.24, Added measure current feature */
#else
	...
	return ICharging;
#endif
/* [PLATFORM]-Add-END by TCTSZ.leo.guo, 2015.07.24 */
}
...
kal_int32 battery_meter_get_battery_temperature(void)
{
    ...
    return batt_temp;
#else
//[FEATURE]-Add-BEGIN by leo.guo,29/05/2015,mini sw pr1032027
...
{
    /* [BUGFIX]-Add-BEGIN by TCTSZ.leo.guo, 07/01/2015, Fixed the report soc to 50%. */
#if defined(SOC_BY_HW_FG)
