...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 6/16/2015, add charge enable node */
#if (defined CONFIG_TCT_CHARGE_CONTROL)
int sys_chg_control = 1;
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */

/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/7/2015, add charge led enable  */
#if (defined CONFIG_TCT_CHARGE_LED_CONTROL)
kal_bool sys_chg_led_control;
int gpio_charge_led_number = GPIO4;

void charge_led_enable(kal_bool enable)
{
	mt_set_gpio_mode(gpio_charge_led_number, GPIO_MODE_GPIO); 
	mt_set_gpio_dir(gpio_charge_led_number, GPIO_DIR_OUT);

	if (enable) {
		mt_set_gpio_out(gpio_charge_led_number, GPIO_OUT_ZERO);

	}
	else {
		mt_set_gpio_out(gpio_charge_led_number, GPIO_OUT_ONE);
	}
}
...
static kal_bool charge_status = KAL_FALSE;
static kal_int32 cur_soc_synchro_time = 0;
static kal_int32 pre_soc_synchro_time = 0;
static kal_int32 synchro_time = 0;

static kal_int32 cur_discharge_synchro_time = 0;
static kal_int32 pre_discharge_synchro_time = 0;
static kal_int32 discharge_synchro_time = 0;
kal_int32 pre_sync_soc = 1;
static kal_int32 soc_synchro_timer = 109;
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */

/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/24/2015, pr-1092300, add battery manufacturer check */
#ifdef TCT_CHECK_BATTERY_TYPE
...
/* [BUGFIX]-Mod-BEGIN by TCTSZ.pingao.yang, 2015/08/05,  calibration battery charge profile */
#ifdef SOC_SYNCHRO_UI_SOC_TIMER
typedef struct{
	kal_int32 fg_real_soc;
	kal_int32 soc_sync_time;
}SOC_SYNC_TIMER;

SOC_SYNC_TIMER soc_time_map_table[] = {
	{1,	 98},
	{70, 100},
	{71, 102},
	{72, 104},
...
static kal_int32 get_sync_soc_time(kal_int32 soc)
{
	kal_int32 i;
	kal_int32 sync_time;
	SOC_SYNC_TIMER *soc_time_table;

	soc_time_table = &soc_time_map_table[0];

	if (soc >= TIMER_SYNC_SOC_THRESHOLD) {
		for(i = 0; i <= 30; i++) {
			if (soc == soc_time_table[i].fg_real_soc) {
				sync_time = soc_time_table[i].soc_sync_time;
				break;
			}
		}
	}
	else {
		sync_time = soc_time_table[0].soc_sync_time;
	}

	printk("[allan] soc=%d, sync_time=%d\n", soc, sync_time);
	return sync_time;
}
#endif
/* [BUGFIX]-Mod-END by TCTSZ.pingao.yang */
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, 06/30/2015,  add ntc simulation function */
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/24/2015, pr-1092300, add battery manufacturer check */
#ifdef TCT_CHECK_BATTERY_TYPE
	.BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_SUDC,
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */
};
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/24/2015, pr-1092300, add battery manufacturer check */
	...
	if (g_fg_battery_id == 0)
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_JN;
	else
		bat_data->BAT_BATTERY_TYPE = POWER_SUPPLY_BATTERY_TYPE_TCL_SUDC;
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */
...
#if defined(CUST_CAPACITY_OCV2CV_TRANSFORM) /*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 11/18/2015, pr-942924, modify soc preserve */
				    ...
				    const char *buf, ssize_t count)
{
	...
	if (enable == 1) {
		...
		printk("<allan 1>  sys_chg_control = %d, bat_charging_state = %d\n", sys_chg_control, BMT_status.bat_charging_state);
	} else if (enable == 0) {
		sys_chg_control = 0;
		BMT_status.bat_charging_state = CHR_ERROR;
		printk("<allan 2>  sys_chg_control = %d, bat_charging_state = %d\n", sys_chg_control, BMT_status.bat_charging_state);
	}
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 6/16/2015,  pr-1025641,  add charge enable led node */
//sys/devices/platform/battery/chg_led_enable
#if (defined CONFIG_TCT_CHARGE_LED_CONTROL)
static ssize_t chg_enable_led_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, ssize_t count)
{
	int enable = simple_strtol(buf, NULL, 10);
	...
	if (enable == 1) {
		sys_chg_led_control = 1;
	} else if (enable == 0) {
		sys_chg_led_control = 0;
	}

	charge_led_enable(sys_chg_led_control);

	return count;
}

static ssize_t chg_enable_led_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	return sprintf(buf, "%d\n", sys_chg_led_control);
}

static DEVICE_ATTR(chg_led_enable, 0644, chg_enable_led_show, chg_enable_led_store);
#endif
/* [BUGFIX]-Add-END by TCTSZ.pingao.yang */
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, 06/30/2015,  modify ntc temperature function */
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, 06/30/2015,  modify ntc temperature function */
	...
	if ((BMT_status.temperature >= batt_cust_data.max_charge_temperature)
			|| ((BMT_status.temperature >= batt_cust_data.max_limit_charge_temperature) && (high_temp_stop_charge == KAL_TRUE))){
#else
	if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
#endif
/* [PLATFORM]-Add-END by TCTSZ.leo.guo */
...
static kal_uint32 mt_temperature_average_method(kal_uint32 *bufferdata, kal_uint32 data,
					    kal_int32 *sum, kal_uint8 temperatureIndex)
{
	kal_uint32 avgdata;
	kal_uint8 i;
	static kal_bool tempBufferFirst = KAL_TRUE;

	if (tempBufferFirst == KAL_TRUE) {
		for (i = 0; i < TEMPERATURE_AVERAGE_SIZE; i++) {
			bufferdata[i] = data;
		}

		tempBufferFirst = KAL_FALSE;
		*sum = data * TEMPERATURE_AVERAGE_SIZE;
	}

	*sum -= bufferdata[temperatureIndex];
	*sum += data;
	bufferdata[temperatureIndex] = data;
	avgdata = (*sum) / TEMPERATURE_AVERAGE_SIZE;

	if (((*sum) % TEMPERATURE_AVERAGE_SIZE) >= (TEMPERATURE_AVERAGE_SIZE / 2))
		avgdata = avgdata + 1;

	for (i = 0; i < TEMPERATURE_AVERAGE_SIZE; i++) {
		battery_xlog_printk(BAT_LOG_CRTI, "bufferdata[%d]= (%d)\n", i, bufferdata[i]);
	}
	battery_xlog_printk(BAT_LOG_CRTI, "bufferdata[%d]= (%d), sum = %d, avgdata = %d\n", temperatureIndex,
			    bufferdata[temperatureIndex], *sum, avgdata);
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao, 10/19/2015,  define tct temperature algorithm */
#ifdef TCT_TEMPERATURE_ALGORITHM
	static kal_int32 batteryTempBuffer[TEMPERATURE_AVERAGE_SIZE] = {25,25,25,25,25,25,25,25};
	static kal_uint8 temperatureIndex = 0;
#else
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 7/15/2015,  pr-1043626,  soc sync time synchro */
#ifdef SOC_SYNCHRO_UI_SOC_TIMER
	if (charge_status == KAL_TRUE) {
		if (BMT_status.temperature <= LOW_TEMPERATURE_SETTING) {
			soc_synchro_timer = LOW_SOC_SYNCHRO_TIMER;
		}
		else {
			//soc_synchro_timer = SOC_SYNCHRO_TIMER;
			soc_synchro_timer = get_sync_soc_time(pre_sync_soc);
		}

		cur_soc_synchro_time = rtc_read_hw_time();
		synchro_time = (cur_soc_synchro_time - pre_soc_synchro_time) % soc_synchro_timer;
		printk("<allan 3>  pre_sync_soc = %d, SOC = %d, sync_time = %d, BMT_status.temperature = %d\n",
			pre_sync_soc, SOC, synchro_time, BMT_status.temperature);

		if ((synchro_time < SYNCHRO_TIMER_DIFF) && (SOC > pre_sync_soc) && (pre_sync_soc != 100)) {
			//printk("<allan 4>""%s pre_sync_soc = %d, SOC = %d\n", __func__, pre_sync_soc, SOC);
			SOC = pre_sync_soc + 1;
			pre_sync_soc = SOC;
			BMT_status.SOC = SOC;
		}
		else if ((synchro_time < SYNCHRO_TIMER_DIFF) && (BMT_status.charger_type != STANDARD_CHARGER)
					&& (SOC < pre_sync_soc) && (pre_sync_soc > 0)) {
			SOC = pre_sync_soc - 1;
			pre_sync_soc = SOC;
			BMT_status.SOC = SOC;
		}
		...
		if (BMT_status.temperature <= LOW_TEMPERATURE_SETTING) {
			soc_synchro_timer = DISCHARGE_LOW_SOC_SYNCHRO_TIMER;
		}
		else {
			soc_synchro_timer = DISCHARGE_SOC_SYNCHRO_TIMER;
		}

		cur_discharge_synchro_time = rtc_read_hw_time();
		discharge_synchro_time = (cur_discharge_synchro_time - pre_discharge_synchro_time)
								% soc_synchro_timer;
		printk("<allan 4>  pre_sync_soc = %d, SOC = %d, discharge_synchro_time = %d, BMT_status.temperature = %d\n",
			pre_sync_soc, SOC, discharge_synchro_time, BMT_status.temperature);

		if ((discharge_synchro_time < DISCHARGE_SYNCHRO_TIMER_DIFF) && (SOC < pre_sync_soc) && (pre_sync_soc > 0)) {
			SOC = pre_sync_soc - 1;
			pre_sync_soc = SOC;
			BMT_status.SOC = SOC;
		}
...
#ifdef CONFIG_TCT_LIMIT_USB_CURRENT
	if (BMT_status.charger_exist == KAL_TRUE) {
		if((BMT_status.charger_type == STANDARD_HOST) && (BMT_status.ICharging >= STANDARD_HOST_CURRENT))
			BMT_status.ICharging = STANDARD_HOST_CURRENT;
	}
#endif
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao, 10/19/2015,  define tct temperature algorithm */
	...
	BMT_status.temperature =
	    mt_temperature_average_method(&batteryTempBuffer[0], temperature, &temperature_sum,
				      temperatureIndex);
	temperatureIndex++;
	if (temperatureIndex >= TEMPERATURE_AVERAGE_SIZE)
		temperatureIndex = 0;
#else
	BMT_status.temperature =
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 7/15/2015,  pr-1043626,  soc sync time synchro */
#ifndef SOC_SYNCHRO_UI_SOC_TIMER
	BMT_status.SOC = SOC;
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */
	BMT_status.ZCV = ZCV;
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.leo.guo, 07/28/2015, Fixed the battery status update */
		else{
			if(BMT_status.bat_charging_state == CHR_ERROR
				&& BMT_status.charger_vol<=batt_cust_data.v_charger_nol
				&& sys_chg_control == 1){
				battery_log(BAT_LOG_CRTI, "[BATTERY]Charger normal voltage, update BMT status !!\r\n");
				BMT_status.bat_charging_state = CHR_PRE;
			}
		}
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
		/* [PLATFORM]-ADD-BEGIN by TCTSZ leo.guo, 06/30/2015,  BATTERY Notify PR-1018754*/
		if(BMT_status.temperature >= batt_cust_data.max_raising_charge_temperature)
		...
		/* [PLATFORM]-ADD-BEGIN by TCTSZ leo.guo, 06/30/2015,  BATTERY Notify PR-1018754*/
		if(BMT_status.temperature <= batt_cust_data.min_dropping_charge_temperature)
...
#ifdef TCT_LOW_VOL_POWER_OFF
	if (BMT_status.bat_vol <= TCT_SHUT_DOWN_VOL) {
		printk("<allan>  BMT_status.bat_vol = %d, Shutdown OS!\n", BMT_status.bat_vol);
		battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
	}
#endif
...
void do_chrdet_int_task(void)
{
	...
	/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao, 09/09/2015,  change charge current */
		...
		if (BMT_status.charger_exist == KAL_TRUE) {
			if (BMT_status.charger_type == STANDARD_CHARGER)
				BMT_status.ICharging = (AC_CHARGER_CURRENT/100 - 15);
			else
				BMT_status.ICharging = (USB_CHARGER_CURRENT/100 -15);
		}
	#endif
...
void BAT_thread(void)
{
	...
	/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 7/15/2015,  pr-1043626,  soc sync time synchro */
	#ifdef SOC_SYNCHRO_UI_SOC_TIMER
		pre_discharge_synchro_time = rtc_read_hw_time();
		BMT_status.SOC = pre_sync_soc;
	#endif
	/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */
	}
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 7/15/2015,  pr-1043626,  soc sync time synchro */
#ifdef SOC_SYNCHRO_UI_SOC_TIMER
	if ((upmu_is_chr_det() == KAL_TRUE) && (charge_status == KAL_FALSE)) {
		charge_status = KAL_TRUE;
		pre_sync_soc = BMT_status.SOC;
		pre_soc_synchro_time = rtc_read_hw_time();
		printk("<allan 1>  pre_sync_soc = %d, charge_status = %d\n", pre_sync_soc, charge_status);
	}
	else if ((upmu_is_chr_det() == KAL_FALSE) && (charge_status == KAL_TRUE)) {
		charge_status = KAL_FALSE;
		pre_sync_soc = BMT_status.SOC;
		pre_discharge_synchro_time = rtc_read_hw_time();
		printk("<allan 2>  pre_sync_soc = %d, charge_status = %d\n", pre_sync_soc, charge_status);
	}
#endif
/*[BUGFIX]-Add-END by TCTSZ.pingao.yang */
...
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 10/16/2015, disable ntc check */
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 09/07/2015, add charge enable node */
	#if (defined CONFIG_TCT_CHARGE_CONTROL)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_chg_enable);
	#endif
	/* [BUGFIX]-Add-END by TCTSZ.pingao.yang */

	/* [PLATFORM]-Add-BEGIN by TCTSZ.pingao.yang, 09/07/2015, add charge led control */
	#if (defined CONFIG_TCT_CHARGE_LED_CONTROL)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_chg_led_enable);
	#endif
	/* [BUGFIX]-Add-END by TCTSZ.pingao.yang */
