...
static int mxc400x_auto_init(void);
static int mxc400x_auto_uninit(void);
...
static struct sensor_init_info mxc400x_init_info =
{
    .name   = MXC400X_DEV_NAME,
    .init   = mxc400x_auto_init,
    .uninit = mxc400x_auto_uninit,
};

static bool isMxc400x = false;
...
static unsigned char mxc4005_chip_id = MXC400X_ID;

extern struct acc_hw* get_cust_acc_hw(void);

#ifdef CONFIG_MTK_AUTO_DETECT_ACCELEROMETER
extern struct acc_hw* get_cust_acc_hw_sub(int id);
#endif
...
static int MXC400X_ReadCalibration(struct i2c_client *client, int dat[MXC400X_AXES_NUM])
{
	...
	#endif
    dat[obj->cvt.map[MXC400X_AXIS_X]] = obj->cvt.sign[MXC400X_AXIS_X]*(obj->offset[MXC400X_AXIS_X]*mul * GRAVITY_EARTH_1000 / obj->reso->sensitivity + obj->cali_sw[MXC400X_AXIS_X]);
    dat[obj->cvt.map[MXC400X_AXIS_Y]] = obj->cvt.sign[MXC400X_AXIS_Y]*(obj->offset[MXC400X_AXIS_Y]*mul * GRAVITY_EARTH_1000 / obj->reso->sensitivity + obj->cali_sw[MXC400X_AXIS_Y]);
    dat[obj->cvt.map[MXC400X_AXIS_Z]] = obj->cvt.sign[MXC400X_AXIS_Z]*(obj->offset[MXC400X_AXIS_Z]*mul * GRAVITY_EARTH_1000 / obj->reso->sensitivity + obj->cali_sw[MXC400X_AXIS_Z]);                        
...
{  
	...
	err = 0;
		...
		mul = obj->reso->sensitivity/mxc400x_offset_resolution.sensitivity;
	#endif
    raw[MXC400X_AXIS_X] = obj->offset[MXC400X_AXIS_X]*mul * GRAVITY_EARTH_1000 / obj->reso->sensitivity + obj->cali_sw[MXC400X_AXIS_X];
	raw[MXC400X_AXIS_Y] = obj->offset[MXC400X_AXIS_Y]*mul * GRAVITY_EARTH_1000 / obj->reso->sensitivity + obj->cali_sw[MXC400X_AXIS_Y];
	raw[MXC400X_AXIS_Z] = obj->offset[MXC400X_AXIS_Z]*mul * GRAVITY_EARTH_1000 / obj->reso->sensitivity + obj->cali_sw[MXC400X_AXIS_Z];
...
static int MXC400X_WriteCalibration(struct i2c_client *client, int dat[MXC400X_AXES_NUM])
{
	...
	obj->offset[MXC400X_AXIS_X] = (s8)(obj->cvt.sign[MXC400X_AXIS_X]*(cali[obj->cvt.map[MXC400X_AXIS_X]]) * obj->reso->sensitivity / GRAVITY_EARTH_1000/(divisor));
	obj->offset[MXC400X_AXIS_Y] = (s8)(obj->cvt.sign[MXC400X_AXIS_Y]*(cali[obj->cvt.map[MXC400X_AXIS_Y]]) * obj->reso->sensitivity / GRAVITY_EARTH_1000/(divisor));
	obj->offset[MXC400X_AXIS_Z] = (s8)(obj->cvt.sign[MXC400X_AXIS_Z]*(cali[obj->cvt.map[MXC400X_AXIS_Z]]) * obj->reso->sensitivity / GRAVITY_EARTH_1000/(divisor));
...
static int MXC400X_ReadSensorDataForDaemon(struct i2c_client *client, int *buf, int bufsize)
{
    ...
    if (sensor_power == false)
    {
        ...
        if (res)
            GSE_ERR("Power on mxc400x error, res = %d\n", res);
        msleep(20);
    }
#endif
...
int gsensor_mxc4005_operate(void* self, uint32_t command, void* buff_in, int size_in,
	 ...
	 switch (command)
	 {
		 ...
		 case SENSOR_ENABLE:
			 ...
			 else
			 {
                ...
                printk("Gsensor device enable function enable = %d, sensor_power = %d!\n",value,sensor_power);
		 ...
		 case SENSOR_GET_DATA:
			 ...
			 else
			 {
				 ...
				 mutex_lock(&mxc400x_mutex);
				 MXC400X_ReadSensorDataForDaemon(priv->client, buff, MXC400X_DATA_LEN);
                 gsensor_data->values[0] = buff[0];
				 gsensor_data->values[1] = buff[1];
				 gsensor_data->values[2] = buff[2];//cali_sensor_data[2];
				 mutex_unlock(&mxc400x_mutex);
...
static long mxc400x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case GSENSOR_IOCTL_GET_STATUS:
             ...
             mutex_unlock(&mxc400x_mutex);
             break;
        case GSENSOR_IOCTL_GET_CHIP_ID:
             data = (void __user *) arg;
             if (data == NULL)
             {
                 err = -EINVAL;
             ...
             if (copy_to_user(data, &mxc4005_chip_id, sizeof(mxc4005_chip_id)))
             {
                 err = -EFAULT;
                 break;
             }
...
static int mxc400x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(atomic_read(&obj->firlen) > 0)
	{
     ...
     #else
     if(err = mxc400x_create_attr(&mxc400x_gsensor_driver.driver))
	 {
		 GSE_ERR("create attribute err = %d\n", err);
		  goto exit_create_attr_failed;
	 }
	 ...
	 sobj.self = obj;
	 sobj.polling = 1;
	 sobj.sensor_operate = gsensor_mxc4005_operate;
...
static int mxc400x_i2c_remove(struct i2c_client *client)
{
   ...
   #else
    if(err = mxc400x_delete_attr(&mxc400x_gsensor_driver.driver))
	{
		 GSE_ERR("mxc400x_delete_attr fail: %d\n", err);
	}
   #endif
...
static int mxc400x_probe(void)
{
#ifdef CONFIG_MTK_AUTO_DETECT_ACCELEROMETER
    struct acc_hw *hw = get_cust_acc_hw_sub(2);
#else
