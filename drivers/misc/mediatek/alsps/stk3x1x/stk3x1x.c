...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.7.1modify 151214"
...
int stk3x1x_read_id(struct i2c_client *client)
{
    ...
    APS_ERR("%s: p_wv_r_bd_with_co = 0x%x\n", __func__, obj->p_wv_r_bd_with_co);
    /*if(buf[0] == 0)
    {
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
    ...
    obj->ps_high_thd_boot = 1700;
    obj->ps_low_thd_boot = 1500;
    atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot);
    atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot);
#endif
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
    ...
    if(diff > obj->stk_max_min_diff) {
		...
		if((obj->psi > 0) && (obj->psi < 100) )
		{
			obj->stk_ht_n_ct = 120;
			obj->stk_lt_n_ct = 70;
		}
		else if((obj->psi >= 100) && (obj->psi < 200))
		{
			obj->stk_ht_n_ct = 130;
			obj->stk_lt_n_ct = 75;
		}
		else if((obj->psi >= 200) && (obj->psi < 500))
		{
			obj->stk_ht_n_ct = 140;
			obj->stk_lt_n_ct = 80;
		}
		else if((obj->psi >= 500) && (obj->psi < 1000))
		{
			obj->stk_ht_n_ct = 150;
			obj->stk_lt_n_ct = 90;
		}
		else if(obj->psi > 1000)
		{
			obj->stk_ht_n_ct = 200;
			obj->stk_lt_n_ct = 100;
		}
...
static ssize_t stk3x1x_show_transmittance(struct device_driver *ddri, char *buf)
{
    int len = -1;
    ...
    len = atomic_read(&stk3x1x_obj->als_transmittance);
    APS_LOG("%s: len = %2d\n", __func__, len);
    return scnprintf(buf, PAGE_SIZE, "%d\n", len);
}

/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_transmittance(struct device_driver *ddri, const char *buf, size_t count)
{
    int value;
    ...
    atomic_set(&stk3x1x_obj->als_transmittance, value);
    return count;
}
/*----------------------------------------------------------------------------*/
...
static DRIVER_ATTR(transmittance,  S_IWUSR | S_IRUGO, stk3x1x_show_transmittance, stk3x1x_store_transmittance);
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
    ...
    */
    als_value_report = als * atomic_read(&obj->als_transmittance) / 1000;
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    atomic_set(&obj->ps_high_thd_val, obj->hw->ps_high_thd_val);
    atomic_set(&obj->ps_low_thd_val, obj->hw->ps_low_thd_val);
    atomic_set(&obj->als_transmittance, obj->hw->als_transmittance);
    ...
    APS_ERR("%s: state_val=0x%x, psctrl_val=0x%x, alsctrl_val=0x%x, ledctrl_val=0x%x, wait_val=0x%x, int_val=0x%x\n",
