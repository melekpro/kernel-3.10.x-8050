...
#endif
#define PFX "ov56702nd_camera_sensor"
#define LOG_INF(format, args...)	pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define OV56702nd_OTP
...
#define OV56702nd_MODULE_ID 0x44
#define OV56701st_MODULE_ID 0x6
...
static int RG_Ratio_Typical = 611;   // 315
static int BG_Ratio_Typical = 611;	//291
...
static int check_otp_info(int index)
{
		...
		if (flag == 0x00) {
			LOG_INF("pxs_ov5670_otp  retun 0,group index is empty; 		  %s \n",__func__); //pei_add				
			return 0;
		}
		else if (flag & 0x02) {
			LOG_INF("pxs_ov5670_otp  return 1,group index has invalid data; %s \n", __func__); //pei_add		
			return 1;
		}
		else {
			LOG_INF("pxs_ov5670_otp  return 2 ,group index has valid data ; %s \n", __func__); //pei_add		
			return 2;
		}
}
...
static int check_otp_wb(int index)
{
		...
		//partial mode OTP write start address
		write_cmos_sensor(0x3d88, 0x70);
		write_cmos_sensor(0x3d89, 0x26);
		// partial mode OTP write end address
		write_cmos_sensor(0x3d8A, 0x70);
		write_cmos_sensor(0x3d8B, 0x26);
		// read otp into buffer
		write_cmos_sensor(0x3d81, 0x01);
		mDELAY(10);

		//select group
		flag = read_cmos_sensor(0x7026);
...
// index: index of otp group. (1, 2, 3)
// otp_ptr: pointer of otps_struct
// return: 0,
static int read_otp_info(int index, struct otps_struct *otp_ptr)
{
		...
		(*otp_ptr).production_year = read_cmos_sensor(start_addr + 4);
		(*otp_ptr).production_month = read_cmos_sensor(start_addr + 5);
		(*otp_ptr).production_day = read_cmos_sensor(start_addr + 6);

		module_id = (*otp_ptr).module_integrator_id;
	    printk("cuckoo pxs_ov5670_otp moduleID is : 0x%x  %s \n",module_id, __func__); //pei_add	
...
// otp_ptr: pointer of otps_struct
// return: 0,
static int read_otp_wb(int index, struct otps_struct *otp_ptr)
{
...
static int update_awb_gain(int R_gain, int G_gain, int B_gain)
{
	...
	LOG_INF("pxs_ov5670_otp AA1=%x ,AA2=%x ,AA3=%x ,AA4=%x ,AA5=%x ,AA6=%x \n ", AA1,AA2,AA3,AA4,AA5,AA6);
	...
	LOG_INF("pxs_ov5670_otp2 AA11=%x ,AA12=%x ,AA13=%x ,AA14=%x ,AA15=%x ,AA16=%x \n ", AA11,AA12,AA13,AA14,AA15,AA16);
...
static int get_moduleID()
{

	struct otps_struct current_otp;
	int i;
	int info_index;
	int temp;
	...
	LOG_INF("pxs_ov5670_otp	info_index=%d,  %s \n",info_index, __func__); //pei_add		
	if (i>3) {
	...
	LOG_INF("pxs_ov5670_otp moduleID is : 0x%x , %s\n",current_otp.module_integrator_id,__func__);
...
static int update_otp_wb()
{
	...
	LOG_INF("pxs_ov5670_otp	otp_index=%d,  %s \n",otp_index, __func__); //pei_add		
	if (i>3) {
		// no valid wb OTP data
	...
	//xuejian.zhong change this 
	if (rg == 0 || bg == 0){
		printk("pxs_ov5670_otp	read wb OTP data error  %s \n", __func__); //pei_add
		return 1;
	}
	// change end 
	//calculate G gain
	...
	LOG_INF("pxs_ov5670_otp	rg=%x,bg=%x,  %s \n",rg,bg, __func__); //pei_add
	LOG_INF("pxs_ov5670_otp  nR_G_gain=%x, nB_G_gain=%x, nG_G_gain=%x,  %s \n",nR_G_gain,nB_G_gain,nG_G_gain, __func__); //pei_add	
	...
	LOG_INF("pxs_ov5670_otp  R_gain=%x, G_gain=%x, B_gain=%x,  %s \n",R_gain ,B_gain,G_gain, __func__); //pei_add	
...
#ifdef OV56702nd_OTP
				if (get_moduleID() != OV56701st_MODULE_ID){
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					return ERROR_NONE;
				}else{
					*sensor_id = 0xFFFFFFFF;
					LOG_INF("error: no match module ID\n");
				}
#else
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
...
/*#ifdef OV56702nd_OTP
	...
	/*xuejian.zhong add OTP function*/
#ifdef OV56702nd_OTP
	update_otp_wb();
#endif
	/* add  end */
