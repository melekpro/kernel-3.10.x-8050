...
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SP2508_RAW_SENSOR_ID,
    ...
    .pre = {
        ...
        .linelength = 2224,             //record different mode's linelength
        .framelength = 1257,            //record different mode's framelength
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
    ...
    switch (image_mirror) {
        case IMAGE_NORMAL:
			      write_cmos_sensor(0x3f, 0x03);    //Set normal
            break;
        case IMAGE_H_MIRROR:
        ...
        case IMAGE_HV_MIRROR:
            write_cmos_sensor(0x3f, 0x00); //Set mirror and flip
            break;
        default:
...
static void sensor_init(void)
{
   ...
   write_cmos_sensor(0x79,0x42);
   write_cmos_sensor(0x85,0x0f);
   write_cmos_sensor(0x09,0x01);
   write_cmos_sensor(0x0a,0x10);
   write_cmos_sensor(0x21,0xef);
   write_cmos_sensor(0x25,0xf2);
   write_cmos_sensor(0x26,0x00);
   write_cmos_sensor(0x2a,0xea);
   write_cmos_sensor(0x2c,0xf0);
   write_cmos_sensor(0x8a,0x55);
   write_cmos_sensor(0x8b,0x55);
   write_cmos_sensor(0x19,0xf3);
   write_cmos_sensor(0x11,0x30);
   write_cmos_sensor(0xd0,0x01);
   write_cmos_sensor(0xd1,0x01);
   write_cmos_sensor(0xd2,0x70);
   ...
   write_cmos_sensor(0x00,0xa6);
   write_cmos_sensor(0x01,0xa6);
   write_cmos_sensor(0x03,0xa6);
   write_cmos_sensor(0x04,0xa6);
 ...
 //from K2
	...
	write_cmos_sensor(0x2f,0x10); //pll clk 84M           0x08 60M 
	write_cmos_sensor(0xfd,0x01);
	write_cmos_sensor(0x03,0x04); //exp time, 3 base
	write_cmos_sensor(0x04,0x3e);
	write_cmos_sensor(0x06,0x10); //vblank
	write_cmos_sensor(0x24,0x80); //pga gain 10x
	...
	write_cmos_sensor(0x00,0x86);
	write_cmos_sensor(0x01,0x86);
	write_cmos_sensor(0x03,0x86);
...
static void preview_setting(kal_uint16 currefps)   //PreviewSetting
{
  ...
  LOG_INF("Enter preview! currefps:%d\n",currefps);
  if(currefps != 0)
  {
...
static void capture_setting(kal_uint16 currefps)  // IMX135MIPI_set_13M
{
	 #if 1
	  kal_uint32 frame_length;
  LOG_INF("Enter capture! currefps:%d\n",currefps);
  if(currefps != 0)
  {
      frame_length = imgsensor_info.pre.pclk / imgsensor.current_fps * 10 / imgsensor_info.pre.linelength;
      imgsensor.dummy_line = (frame_length > imgsensor_info.pre.framelength) ? (frame_length - imgsensor_info.pre.framelength) : 0;
