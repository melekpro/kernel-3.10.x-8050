...
static int s61x_init_flag = 0;  // 0 = OK, -1 = fail
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info s61x_init_info = {
        .name   = S61X_DEV_NAME,
...
static int I2C_RxData(char *rxData, int length)
{
        ...
        if (atomic_read(&data->trace) & SS_I2C_DEBUG) {
                printk(KERN_INFO "S61X RxData len=%02x addr=%02x\n data=", length, addr);
                for (i = 0; i < length; i++) {
...
#if DEBUG
        if (atomic_read(&data->trace) & SS_I2C_DEBUG) {
                printk(KERN_INFO "S61X TxData len=%02x addr=%02x\n data=", length, txData[0]);
...
#if defined(ADVAN92_6922)||defined(MIKI92_6922)
        sprintf(strbuf, "s61xd6922");
#else        
...
static int __init s61x_init(void)
{
        ...
        hwmsen_msensor_add(&s61x_init_info);
        return 0;
}
