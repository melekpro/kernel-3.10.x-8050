...
u32 pinSet[3][8] = {
    ...
    if (On) {
        ...
        if (currSensorName && pinSetIdx == 1 && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV5670_MIPI_RAW) || 0 == strcmp(currSensorName, SENSOR_DRVNAME_OV56702ND_MIPI_RAW)))
        {
...
#else
				if(mt_set_gpio_mode(GPIO_CAMERA_DVDD12_EN,GPIO_MODE_00)){PK_DBG("[CAMERA LENS] set gpio mode failed!! (DVDD12)\n");}
				if(mt_set_gpio_dir(GPIO_CAMERA_DVDD12_EN,GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (DVDD12)\n");}
				if(mt_set_gpio_out(GPIO_CAMERA_DVDD12_EN,GPIO_OUT_ONE)){PK_DBG("[CAMERA LENS] set gpio failed!! (DVDD12)\n");}
#endif
		...
		else if (currSensorName && pinSetIdx == 0 && (0 == strcmp(SENSOR_DRVNAME_OV8858_MIPI_RAW, currSensorName)) || (0 == strcmp(SENSOR_DRVNAME_OV8858ZGX_MIPI_RAW, currSensorName)))
		{
        	// disable inactive sensor
        	  ISP_MCLK1_EN(1);
			  ...
			  if (GPIO_CAMERA_INVALID != pinSet[1 - pinSetIdx][IDX_PS_CMPDN]) {
				  if(mt_set_gpio_mode(pinSet[1 - pinSetIdx][IDX_PS_CMPDN],pinSet[1 - pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
				  if(mt_set_gpio_dir(pinSet[1 - pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
				  if(mt_set_gpio_out(pinSet[1 - pinSetIdx][IDX_PS_CMPDN],GPIO_OUT_ZERO)){PK_DBG("[CAMERA LENS] set gpio failed!! (CMPDN)\n");}
			  }
			  ...
			  if (GPIO_CAMERA_INVALID != pinSet[1 - pinSetIdx][IDX_PS_CMRST]) {
				  if(mt_set_gpio_mode(pinSet[1 - pinSetIdx][IDX_PS_CMRST],pinSet[1 - pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (CMRST)\n");}
				  if(mt_set_gpio_dir(pinSet[1 - pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (CMRST)\n");}
				  if(mt_set_gpio_out(pinSet[1 - pinSetIdx][IDX_PS_CMRST],GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set gpio failed!! (CMRST)\n");}
			  }
#endif
            ...
            if(mt_set_gpio_mode(GPIO_CAMERA_AF_EN_PIN,GPIO_MODE_00)){PK_DBG("[CAMERA SENSOR] set AF_EN_PIN mode failed!!\n");}
            if(mt_set_gpio_dir(GPIO_CAMERA_AF_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set AF_EN_PIN dir failed!!\n");}
            if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[CAMERA SENSOR] set AF_EN_PIN failed!!\n");}     
            ...
            //[Timming Modify :chengming.xiang 2015-11-03 Beging ]
            //PWDN or RST Pin Should connect to IOVDD  
            mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_OUT_ONE);
            ////[Timming Modify :chengming.xiang 2015-11-03 End ]
            mdelay(1);
            if(TRUE != _hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200, &regVCAMD))
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAM_D), power id = %s \n", CAMERA_POWER_VCAM_D);
            ...
            if(TRUE != _hwPowerOn(CAMERA_POWER_VCAM_AF, VOL_2800, &regVCAMAF))
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (vcam_af), power id = %s \n", CAMERA_POWER_VCAM_D);
                 goto _kdCISModulePowerOn_exit_;
            }  
			mdelay(1);
            mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_OUT_ONE);
            mdelay(5);
        }
        else if (currSensorName && pinSetIdx == 0 && (0 == strcmp(SENSOR_DRVNAME_OV2680_MIPI_RAW ,currSensorName))) 
        {
          ...
          ISP_MCLK1_EN(1);
		    ...
		    else if (currSensorName && pinSetIdx == 1 && (0 == strcmp(SENSOR_DRVNAME_SP0A20_YUV ,currSensorName))) 
        {
		    ...
		    else if (currSensorName && pinSetIdx == 0 && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW ,currSensorName))) 
        {
          ISP_MCLK1_EN(1);
			    // add end
			    if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
        ...
        if (currSensorName && pinSetIdx == 1 && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV5670_MIPI_RAW) || 0 == strcmp(currSensorName, SENSOR_DRVNAME_OV56702ND_MIPI_RAW))) {
            ...
            if(TRUE != _hwPowerDown(CAMERA_POWER_VCAM_D, &regVCAMD))
            {
				...
				if(mt_set_gpio_mode(GPIO_CAMERA_DVDD12_EN,GPIO_MODE_00)){PK_DBG("[CAMERA LENS] set gpio mode failed!! (DVDD12)\n");}
				if(mt_set_gpio_dir(GPIO_CAMERA_DVDD12_EN,GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (DVDD12)\n");}
				if(mt_set_gpio_out(GPIO_CAMERA_DVDD12_EN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA LENS] set gpio failed!! (DVDD12)\n");}
#endif
 ...
 //VCAM_A
        ...
        {   
        	  ISP_MCLK1_EN(0);
	          mdelay(1);
	    // 2015-10-14  chengming.xiang modify begin :
            // modify OV8858 power off timming as OV8858 Spec Page 31 power down sequence case 1(dovdd falling first),PWDN connect to DOVDD:
            // RESET_PIN Falling to Zero -> power off DVDD -> power off IOVDD -> power off AVDD		      
            if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (CMRST)\n");}
            if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (CMRST)\n");}
            if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set gpio failed!! (CMRST)\n");}			      
            ...
            if(mt_set_gpio_mode(GPIO_CAMERA_AF_EN_PIN,GPIO_MODE_00)){PK_DBG("[CAMERA SENSOR] set AF_EN_PIN mode failed!!\n");}
            if(mt_set_gpio_dir(GPIO_CAMERA_AF_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set AF_EN_PIN dir failed!!\n");}
            if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set AF_EN_PIN failed!!\n");}     
            mdelay(2);
            if(TRUE != _hwPowerDown(CAMERA_POWER_VCAM_D, &regVCAMD))
            {
                 PK_DBG("[CAMERA SENSOR] Fail to OFF core power (VCAM_D), power id = %s \n",CAMERA_POWER_VCAM_D);
	    ...
	    if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
            ...
            // chengming.xiang 2015-10-14 modify end 

            //vcam_af
            if(TRUE != _hwPowerDown(CAMERA_POWER_VCAM_AF, &regVCAMAF)) {
                PK_DBG("[CAMERA SENSOR] Fail to OFF digital power (VCAM_AF), power id = %s \n", CAMERA_POWER_VCAM_AF);
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }            
