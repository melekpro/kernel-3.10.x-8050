...
void (*ctp_charge_mode_enable)(void) = NULL;
EXPORT_SYMBOL(ctp_charge_mode_enable);
...
void homekey_int_handler(void)
{
    ...
    if(pmic_get_register_value(PMIC_FCHRKEY_DEB)==1)
    {
		...
		pmic_set_register_value(PMIC_RG_FCHRKEY_INT_SEL, 0);
    }
    else
    {
        PMICLOG("[homekey_int_handler] Press HomeKey\r\n");
        kpd_pmic_rstkey_handler(0x1);
		pmic_set_register_value(PMIC_RG_FCHRKEY_INT_SEL, 1);
    }    
#else    
...
void chrdet_int_handler(void)
{
  ...
  if (ctp_charge_mode_enable != NULL)
       ctp_charge_mode_enable();
}
...
void PMIC_CUSTOM_SETTING_V1(void)
{
    ...
    #else    
/*[BUGFIX]-Add-BEGIN by TCTSZ.pingao.yang, 9/9/2015, turn on ldo vcn28 */
