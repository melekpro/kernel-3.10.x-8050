...
static void msdc_emmc_power(struct msdc_host *host,u32 on)
{
	...
	/*[BUGFIX]-Mod-BEGIN by TCTSZ.(gaoxiang.zou@tcl.com), PR1694806 03/17/2016*/
  //  msdc_ldo_power(1, MSDC_VEMC33, 3000000, &g_msdc0_flash);
	if (host->mmc->card && (host->mmc->card->cid.manfid == 0x1f)){
    	msdc_ldo_power(1, MSDC_VEMC33, 3000000, &g_msdc0_flash);
		printk("gcai emcp always on vcc power ----------- \n");
		}
	else
		{
		msdc_ldo_power(on, MSDC_VEMC33, 3000000, &g_msdc0_flash);
		printk("toshiba emcp or other emcp----------- \n");
	}
	/*[BUGFIX]-Mod-END by TCTSZ.(gaoxiang.zou@tcl.com), PR1694806*/
...
static bool msdc_check_written_data(struct mmc_host *mmc,struct mmc_request *mrq)
{
    ...
    if(mmc->card)
        ...
        return 0;
//[FIXBUG]Marked-begin by SZTCT.leo.guo 08.17.2015, Fixed TF card write error issue.
#if 0
    if((host->hw->host_function == MSDC_SD)
        && (host->sclk > 100000000)
        && mmc_card_sd(card)
...
#endif
//[FIXBUG]Marked-end by SZTCT.leo.guo 08.17.2015
    return 0;
}
...
//[PLATFORM]Added-begin by SZTCT.leo.guo 06.23.2015, Fixed TF card hotplug issue.
#ifdef CFG_DEV_MSDC1_CORRECTION
    printk(KERN_CRIT "Fixed msdc configuration correction. ");
    mt_set_gpio_mode(GPIO_MSDC1_INSI, GPIO_MODE_06);
    //mt_set_gpio_dir(GPIO_MSDC1_INSI, GPIO_DIR_OUT);
	//mt_set_gpio_out(GPIO_MSDC1_INSI, 1);
    mt_set_gpio_pull_enable(GPIO_MSDC1_INSI, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_select(GPIO_MSDC1_INSI, GPIO_PULL_UP);
#endif
//[PLATFORM]Added-end by SZTCT.leo.guo 06.23.2015
