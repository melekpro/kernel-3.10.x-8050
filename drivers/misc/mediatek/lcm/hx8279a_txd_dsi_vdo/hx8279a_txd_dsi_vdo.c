...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dsi.PLL_CLOCK = 286;		//288, div by 26
}
...
static void lcm_init(void)
{
	...
	mt_set_gpio_mode(GPIO_LCM_DSI_TE_PIN, GPIO_MODE_00);
...
static unsigned int lcm_compare_id(void)
{
	unsigned int id1_state = 0;
        ...
        if ((id1_state == 1) && (id2_state == 0))
        {
                LCM_PRINT("[LCD]hx8279a_txd lcd selected!\n");
                lcm_1v8_power_off();
                return 1;
        }
        else
...
LCM_DRIVER hx8279a_txd_dsi_vdo_lcm_drv = 
{
    .name			= "hx8279a_txd_dsi_vdo",
