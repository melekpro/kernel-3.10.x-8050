...
u8 driver_num = 0;
u8 sensor_num = 0;
int gtp_ref_retries = 0;
int gtp_clk_retries = 0;
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    {
#if GTP_GESTURE_WAKEUP
        if (gesture_wakeup_flag != 0 && DOZE_ENABLED == doze_status)
        {
            return ret;
        }
#endif
...
static u8 gtp_main_clk_proc(struct i2c_client *client)
{
    ...
    GTP_DEBUG("[gtp_main_clk_proc]Waiting for FS %d", gtp_clk_retries);
    if (gup_check_fs_mounted("/data") == FAIL)
    {
...
static void force_reset_guitar(void)
{
    ...
    ret += regulator_enable(tpd->reg);
    if (ret)
        GTP_INFO("Power on or Power off ic error!");
#else
    // Power off TP
#ifdef TPD_POWER_SOURCE_CUSTOM
...
#if GTP_PALM_SUSPEND
        if (palm_suspend_flag)
        {
            if (((finger & 0x40) == 0x40) && tpd_halt == 0)
            {
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type && !gesture_wakeup_flag)
    {
        u8 opr_buf[2] = {0};
