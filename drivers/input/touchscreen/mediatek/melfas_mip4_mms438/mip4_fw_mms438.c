...
//                goto ERROR;
//            }
            printk("chip fw version 0x%04x 0x%04x firmare fw version 0x%04x 0x%04x\n",ver_chip[2], ver_chip[3], bin_info->ver_app, bin_info->ver_param);
            //Compare version
            if((ver_chip[2] >= bin_info->ver_app) && (ver_chip[3] >= bin_info->ver_param)) {
                dev_info(&client->dev, "%s - Chip firmware is already up-to-date\n", __func__);
                printk("%s - Chip firmware is already up-to-date\n", __func__);
                ret = fw_err_uptodate;
                goto ERROR;
            }
        }
...
void mip_kernel_fw_force_update_controller(struct mip_ts_info *info, const struct firmware *fw, struct i2c_client *client) {
    int retry = 8;
    int ret;
