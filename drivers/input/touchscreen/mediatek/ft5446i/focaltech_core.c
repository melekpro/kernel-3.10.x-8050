...
static bool is_esd_ready = false;
void esd_switch(int on);
EXPORT_SYMBOL(esd_switch);
#endif
...
#ifdef FTS_PALM_SUSPEND
extern bool palm_suspend_flag;
#endif
#if FTS_GESTRUE_EN
extern int gesture_wakeup_flag;
#endif

#ifdef FTS_SMART_COVER_SWITCH
extern int smart_cover_flag;
extern void (*hall_state_charge_notify)(int state);
void focaltech_smart_cover_switch(int state);
extern void (*smart_cover_flag_charge_notify)(int flag);
void focaltech_smart_cover_flag_charge(int flag);
int focaltech_cover_state = 1;
#endif
...
#ifdef FTS_SMART_COVER_SWITCH
void focaltech_update_smart_cover_state(void) {
    int ret = 0;
    u8 val = 0;
    printk("\n %s smart_cover_flag = %d focaltech_cover_state = %d \n", __func__, smart_cover_flag, focaltech_cover_state);
    if(smart_cover_flag == 0) {
        fts_write_reg(fts_i2c_client, 0xC1, 0);
        printk("fts_write_reg 0xc1 0x00\n");
        return;
    }

    if(focaltech_cover_state) {
        ret = fts_write_reg(fts_i2c_client, 0xC1, 0);
    } else {
        ret = fts_write_reg(fts_i2c_client, 0xC1, 1);
    }
    printk(" %s ret = %d\n", __func__, ret);
    fts_read_reg(fts_i2c_client, 0xC1, &val);
    printk(" %s val = %d\n", __func__, val);
}
...
void focaltech_smart_cover_flag_charge(int flag)
{
    ...
    if(smart_cover_flag == 0) {
        ret = fts_write_reg(fts_i2c_client, 0xC1, 0);
    } else if(smart_cover_flag == 1 && focaltech_cover_state == 0){
        ret = fts_write_reg(fts_i2c_client, 0xC1, 1);
    }
    printk(" %s ret = %d\n", __func__, ret);
    fts_read_reg(fts_i2c_client, 0xC1, &val);
...
void focaltech_smart_cover_switch(int state)
{
    ...
    focaltech_cover_state = state;
    focaltech_update_smart_cover_state();
}
#endif
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
    ...
    //FTS_DBG("cinfo->x[0] = %d, cinfo->y[0] = %d, cinfo->p[0] = %d\n", cinfo->x[0], cinfo->y[0], cinfo->p[0]);
    return true;
};
...
static int fts_report_value(struct ts_event *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (data->au8_touch_event[i] == 0 || data->au8_touch_event[i] == 2) {
            ...
            //FTS_DBG("%s:x[%d] =%d,y[%d]= %d",, __func__, i,data->au16_x[i],i,data->au16_y[i]);
    ...
    //FTS_DBG("tpd D x =%d,y= %d",event->au16_x[0],event->au16_y[0]);
    return 0;
}
#endif
...
static int tpd_enable_ps(int enable)
{
    ...
    TPD_PROXIMITY_DEBUG("%s:read 0xb0's value is 0x%02X\n", __func__, state);
    ...
    if (ret < 0) {
        TPD_PROXIMITY_DEBUG("[Focal][Touch] write value fail");
        //return ret;
    }
...
static int touch_event_handler(void *unused)
{
...
#if (FTS_GESTRUE_EN || defined(FTS_PALM_SUSPEND))
    u8 state;
    int ret = 0;
#endif
#ifdef TPD_PROXIMITY
    ...
    do {
        ...
        if (gesture_wakeup_flag && gesture_enabled) {
            ...
            FTS_DBG("read Gestrue data state = %d\n", state);
        ...
        if (palm_suspend_flag && (tpd_halt == 0)) {
            state = 0;
            ret = fts_read_reg(fts_i2c_client, 0x01, &state);

            if (ret < 0) {
                FTS_ERROR("Failed to read large touch status!\n");

            } else if (state == 0x4a) {
                FTS_INFO("Large touch was detected!\n");
...
reset_proc:
    ...
    FTS_DBG("Get report rate is %dHz.\n", uc_reg_value * 10);
    uc_reg_addr = FTS_REG_FW_VER;
    fts_i2c_write(fts_i2c_client, &uc_reg_addr, 1);
    fts_i2c_read(fts_i2c_client, &uc_reg_addr, 0, &uc_reg_value, 1);
    gtp_fw_version = uc_reg_value;
    FTS_DBG("Firmware version = 0x%x\n", uc_reg_value);
    uc_reg_addr = FTS_REG_CHIP_ID;
    fts_i2c_write(fts_i2c_client, &uc_reg_addr, 1);
    retval = fts_i2c_read(fts_i2c_client, &uc_reg_addr, 0, &uc_reg_value, 1);
    FTS_DBG("Chip id is 0x%x.\n", uc_reg_value);
    uc_reg_addr = FTS_REG_VENDOR_ID;
    fts_i2c_write(fts_i2c_client, &uc_reg_addr, 1);
    retval = fts_i2c_read(fts_i2c_client, &uc_reg_addr, 0, &uc_reg_value, 1);
    gtp_sensor_id = uc_reg_value;
    FTS_DBG("Vendor id is 0x%x.\n", uc_reg_value);

    if(retval < 0) {
        uc_reg_value = 0xFE;
        FTS_ERROR("Read I2C error driver don't load!\n");
        return -1;
    }
...
#ifdef FTS_SMART_COVER_SWITCH
    hall_state_charge_notify = focaltech_smart_cover_switch;
    smart_cover_flag_charge_notify = focaltech_smart_cover_flag_charge;
#endif
...
static int tpd_local_init(void)
{
    ...
    if(tpd_load_status == 1) {
        printk("Focaltech === add touch panel driver already.\n");
        return -1;
    }
