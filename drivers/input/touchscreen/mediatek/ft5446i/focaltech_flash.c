...
#include "./code/first/cfg.i"
};
static unsigned char CTPM_FW_2[] = {
#include "./code/second/cfg.i"
};
static struct fts_fw_info fts_all_fw_info[] = {
    {0x82, sizeof(CTPM_FW_1), &CTPM_FW_1[0]},
    {0x34, sizeof(CTPM_FW_2), &CTPM_FW_2[0]},
};
static u16 fts_fw_len = 0;
static unsigned char *CTPM_FW = NULL;
static void fts_get_fw_and_len(u8 vendor_id)
{
    unsigned int i, size;

    size = sizeof(fts_all_fw_info) / sizeof(fts_all_fw_info[0]);
    if (size == 0) {
        FTS_ERROR("FW info was empty!\n");
        return;
    }

    for (i = 0; i < size; i++) {
        if (vendor_id == fts_all_fw_info[i].vendor_id) {
            fts_fw_len = fts_all_fw_info[i].len;
            CTPM_FW = fts_all_fw_info[i].data;
            return;
        }
    }

    FTS_ERROR("Can't find a right fw info for the given vendor!\n");
}
#else
...
#if FTS_COMPATIBLE_MODE
    if (force_updated) {
        fts_ctpm_fw_upgrade_ReadVendorID(fts_i2c_client, &vendor_id);
        printk("%s vendor_id = %x\n", __func__, vendor_id);
        if (vendor_id == 0 || vendor_id != gtp_sensor_id)
            gtp_sensor_id = vendor_id;
    }
    fts_get_fw_and_len(gtp_sensor_id);
#endif
}
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
    ...
    FTS_DBG("Step 6: read out checksum\n");
    reg_addr = 0xCC;
    fts_i2c_read(client, &reg_addr, 1, reg_val_i2c, 1);
    auc_i2c_write_buf[0] = 0x64;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(300);
    temp = 0;
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    //	)
    printk("[%s]:uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
           __func__, uc_tp_fm_ver, uc_host_fm_ver);

    if ((uc_tp_fm_ver < uc_host_fm_ver) || force_updated) {
        msleep(100);
