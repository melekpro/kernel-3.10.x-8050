...
//gesture_wakeup_flag add by tcl
extern int gesture_wakeup_flag;
static int gestureWakeUpStatus = 0;
...
#if defined (SYNAPTICS_SMART_COVER_SWITCH)
#define GLOVER_FINGER_MODE_ON 1
#define GLOVER_FINGER_MODE_OFF 0
#define GLOVER_FINGER_MODE_ADDR 0x042D
extern int smart_cover_flag;
extern void (*hall_state_charge_notify)(int state);
void synaptics_smart_cover_switch(int state);
extern void (*smart_cover_flag_charge_notify)(int flag);
void synaptics_smart_cover_flag_charge(int flag);
int synaptics_cover_state = 1;
#endif
...
static int synaptics_rmi4_i2c_read(struct synaptics_rmi4_data *rmi4_data,
    ...
    if (retry == SYN_I2C_RETRY_TIMES) {
        ...
        SYNA_ERROR("%s: I2C read over retry limit\n", __func__);
...
static int synaptics_rmi4_i2c_write(struct synaptics_rmi4_data *rmi4_data,
    ...
    if (retry == SYN_I2C_RETRY_TIMES) {
        dev_err(&rmi4_data->i2c_client->dev,
                ...
                __func__);
        SYNA_ERROR("%s: I2C write over retry limit\n", __func__);
        retval = -EIO;
    }
...
static int synaptics_rmi4_f11_abs_report(struct synaptics_rmi4_data *rmi4_data,
    ...
    if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture && gesture_wakeup_flag) {
        retval = synaptics_rmi4_i2c_read(rmi4_data,
        ...
        if (detected_gestures) {
           ...
           input_report_key(rmi4_data->input_dev, KEY_LIGHTUP, 1);
           input_sync(rmi4_data->input_dev);
           input_report_key(rmi4_data->input_dev, KEY_LIGHTUP, 0);
           input_sync(rmi4_data->input_dev);
#else
...
flash_prog_mode:
    ...
    SYNA_INFO("synaptics rmi4 query device firmware ID = %d\n", rmi4_data->firmware_id);
    ...
    SYNA_INFO("f11_wakeup_gesture = %d f12_wakeup_gesture = %d\n",
		                                     rmi4_data->f11_wakeup_gesture,
		                                     rmi4_data->f12_wakeup_gesture);
    if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data) {
    ...
    if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
#ifdef CUSTOM_LIGHTUP_SCREEN
	set_bit(KEY_LIGHTUP, rmi4_data->input_dev->keybit);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_LIGHTUP);
#else
...
static void synaptics_hard_reset(struct synaptics_rmi4_data *rmi4_data) {
    SYNA_INFO("synaptics hard reset");
...
#if defined (SYNAPTICS_SMART_COVER_SWITCH)
    hall_state_charge_notify = synaptics_smart_cover_switch;
    smart_cover_flag_charge_notify = synaptics_smart_cover_flag_charge;
#endif
...
static void synaptics_rmi4_f11_readReg(struct synaptics_rmi4_data *rmi4_data) {
    int retval;
    ...
    SYNA_INFO("synaptics_rmi4_f11_readReg full_addr.ctrl_base = 0x%x reporting_control  = 0x%x \n", (int)fhandler->full_addr.ctrl_base, (int)reporting_control);
    if (retval < 0) {
        dev_err(&(rmi4_data->input_dev->dev),
...
static void synaptics_rmi4_f12_readReg(struct synaptics_rmi4_data *rmi4_data) {
	int retval;
    unsigned char offset;
    ...
    SYNA_INFO("synaptics_rmi4_f12_readReg full_addr.ctrl_base + offset = 0x%x reporting_control[0]  = 0x%x [1] = 0x%x [2] = 0x%x \n",
                                                             (int)(fhandler->full_addr.ctrl_base + offset),
                                                             (int)reporting_control[0],
                                                             (int)reporting_control[1],
                                                             (int)reporting_control[2]);
    if (retval < 0) {
        dev_err(&(rmi4_data->input_dev->dev),
                "%s: Failed to change reporting mode\n",
                __func__);
...
static void synaptics_rmi4_f11_wg(struct synaptics_rmi4_data *rmi4_data,
    ...
    SYNA_INFO("read f11 fhandler->full_addr.ctrl_base = 0x%x reporting_control  = 0x%x \n", (int)fhandler->full_addr.ctrl_base, (int)reporting_control);
    ...
    retval = synaptics_rmi4_i2c_write(rmi4_data,
                                      ...
                                      &reporting_control,
                                      sizeof(reporting_control));
	SYNA_INFO("write f11 reporting_control  = 0x%x \n", (int)reporting_control);
    if (retval < 0) {
        dev_err(&(rmi4_data->input_dev->dev),
                "%s: Failed to change reporting mode\n",
                __func__);
...
static void synaptics_rmi4_f12_wg(struct synaptics_rmi4_data *rmi4_data,
    ...
    SYNA_INFO("read f12 fhandler->full_addr.ctrl_base + offset = 0x%x reporting_control[0]  = 0x%x [1] = 0x%x [2] = 0x%x \n",
		                                                  (int)(fhandler->full_addr.ctrl_base + offset),
		                                                  (int)reporting_control[0],
		                                                  (int)reporting_control[1],
		                                                  (int)reporting_control[2]);
    ...
    retval = synaptics_rmi4_i2c_write(rmi4_data,
                                      ...
                                      reporting_control,
                                      sizeof(reporting_control));
    SYNA_INFO("write f12 reporting_control[0]  = 0x%x [1] = 0x%x [2] = 0x%x \n",
                                                                (int)reporting_control[0],
                                                                (int)reporting_control[1],
                                                                (int)reporting_control[2]);
    if (retval < 0) {
...
static void synaptics_rmi4_wakeup_gesture(struct synaptics_rmi4_data *rmi4_data,
    ...
    SYNA_INFO("f11_wakeup_gesture  %d f12_wakeup_gesture = %d enable = %d gesture_wakeup_flag = %d \n", rmi4_data->f11_wakeup_gesture, rmi4_data->f12_wakeup_gesture, enable, gesture_wakeup_flag);
...
static void synaptics_charger_switch(struct synaptics_rmi4_data *rmi4_data) {
    ...
    if (chr_status) {   // charger plugged in
        //if (!chr_pluggedin ) {
            ...
            SYNA_INFO("synaptics_charger_switch CHARGER_PLUG_IN charger_ctrl = 0x%x \n", charger_ctrl);
            //charger_ctrl = (charger_ctrl  | CHARGER_PLUG_IN);
              charger_ctrl |= (1 << CHARGER_PLUG_BIT);
       //     chr_pluggedin = 1;
       // } else
       //     return;
       ...
       // if (chr_pluggedin ) {
            ...
            SYNA_INFO("synaptics_charger_switch CHARGER_PLUG_OUT charger_ctrl = 0x%x \n", charger_ctrl);
    ...
    SYNA_INFO("synaptics_charger_switch charger_ctrl = 0x%x \n", charger_ctrl);
    return;
...
#if defined (SYNAPTICS_SMART_COVER_SWITCH)
int synaptics_rmi4_f11_smart_cover_switch(struct synaptics_rmi4_data *rmi4_data,
         bool enable)
{
     int retval;
     unsigned char glover_write_finger_ctrl=0;
	 unsigned char glover_read_finger_ctrl=0;
     if (enable)
     {
         glover_write_finger_ctrl = GLOVER_FINGER_MODE_ON;
         retval = synaptics_rmi4_i2c_write(rmi4_data,
             GLOVER_FINGER_MODE_ADDR, &glover_write_finger_ctrl, sizeof(glover_write_finger_ctrl));
         if (retval < 0) {
             printk("%s: Failed to switch on glover finger mode\n",
                 __func__);
             return -1;
         }
     }
     ...
     {
         glover_write_finger_ctrl = GLOVER_FINGER_MODE_OFF;
         retval = synaptics_rmi4_i2c_write(rmi4_data,
             GLOVER_FINGER_MODE_ADDR, &glover_write_finger_ctrl, sizeof(glover_write_finger_ctrl));
         if (retval < 0) {
             printk("%s: Failed to switch off glover finger mode\n",
                 __func__);
             return -1;
         }
     }
	 ...
	 retval = synaptics_rmi4_i2c_read(rmi4_data,
                                             GLOVER_FINGER_MODE_ADDR,
                                             &glover_read_finger_ctrl,
                                             sizeof(glover_read_finger_ctrl));
	 SYNA_INFO("glover_write_finger_ctrl = %d glover_read_finger_ctrl = %d\n",
                glover_write_finger_ctrl, glover_read_finger_ctrl);
     return 0;
}

void synaptics_update_smart_cover_state(void) {
   int ret;
   struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);

   SYNA_INFO("\n %s smart_cover_flag = %d synaptics_cover_state = %d \n", __func__, smart_cover_flag, synaptics_cover_state);
   if(rmi4_data == NULL) {
       return;
   }
   ...
   if(synaptics_cover_state) {
       ret = synaptics_rmi4_f11_smart_cover_switch(rmi4_data, 0);
   } else {
       ret = synaptics_rmi4_f11_smart_cover_switch(rmi4_data, 1);
   }
   printk(" %s ret = %d\n", __func__, ret);
}
...
void synaptics_smart_cover_flag_charge(int flag)
{
    ...
    if(smart_cover_flag == 0) {
       ret = synaptics_rmi4_f11_smart_cover_switch(rmi4_data, 0);
    } else if(smart_cover_flag == 1 && synaptics_cover_state == 0){
       ret = synaptics_rmi4_f11_smart_cover_switch(rmi4_data, 1);
    }
	printk(" %s ret = %d\n", __func__, ret);
}
...
void synaptics_smart_cover_switch(int state)
{
    printk("%s smart_cover_flag = %d state = %d \n", __func__, smart_cover_flag, state);
    synaptics_cover_state = state;
    synaptics_update_smart_cover_state();
}
#endif
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h) {
    ...
    SYNA_INFO("early_suspend enable_wakeup_gesture = %d gesture_wakeup_flag = %d \n", rmi4_data->enable_wakeup_gesture, gesture_wakeup_flag);
...
static void synaptics_rmi4_late_resume(struct early_suspend *h) {
    ...
    SYNA_INFO("late_resume enable_wakeup_gesture = %d gesture_wakeup_flag = %d \n", rmi4_data->enable_wakeup_gesture, gesture_wakeup_flag);
...
static int synaptics_rmi4_suspend(struct device *dev) {
    ...
    SYNA_INFO("suspend stay_awake = %d \n", rmi4_data->stay_awake);
    ...
    gestureWakeUpStatus = (rmi4_data->enable_wakeup_gesture && gesture_wakeup_flag);
    SYNA_INFO("suspend enable_wakeup_gesture = %d gesture_wakeup_flag = %d gestureWakeUpStatus = %d \n", rmi4_data->enable_wakeup_gesture, gesture_wakeup_flag, gestureWakeUpStatus);

    if (gestureWakeUpStatus) {
        synaptics_rmi4_wakeup_gesture(rmi4_data, true);
        goto exit;
    }
...
static int synaptics_rmi4_resume(struct device *dev) {
    ...
    SYNA_INFO("resume staying_awake = %d \n", rmi4_data->staying_awake);
    if (rmi4_data->staying_awake)
        return 0;

    if (rmi4_data->f11_wakeup_gesture)
        synaptics_rmi4_f11_readReg(rmi4_data);

    if (rmi4_data->f12_wakeup_gesture)
        synaptics_rmi4_f12_readReg(rmi4_data);

    SYNA_INFO("resume enable_wakeup_gesture = %d gesture_wakeup_flag = %d \n", rmi4_data->enable_wakeup_gesture, gesture_wakeup_flag);

    if(gestureWakeUpStatus != (rmi4_data->enable_wakeup_gesture && gesture_wakeup_flag)) {
       printk("synaptics_rmi4_resume this may cause tp issue\n");
       printk("gestureWakeUpStatus 1 = %d\n", gestureWakeUpStatus);
       printk("gestureWakeUpStatus 2 = %d\n", (rmi4_data->enable_wakeup_gesture && gesture_wakeup_flag));
    }
...
static int tpd_local_init(void) {
    ...
    if (tpd_load_status == 1) {
        printk("synaptics === add touch panel driver already.\n");
        return -1;
    }
    ...
    if (tpd_load_status == 0) { //if(tpd_load_status == 0) // disable auto load touch driver for linux3.0 porting
        printk("add error synaptics touch panel driver.\n");
